...
function getAjaxJson() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            document.querySelector('.say').innerText = data.name;  
            document.querySelector('.go').innerText = data.balance;  
        }
    }
...
async function fetchJson() {
    ...
    document.querySelector('.say').innerText = data.say;  
    document.querySelector('.go').innerText = data.go;  
}